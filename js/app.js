...
async function loadWorld() {
  try {
    ...
    if (w.provinces) {
      ...
      w.provinces.forEach(p => {
        ...
        if (!existing.has(p.name) && btnFit) {
          ...
          btn.className='btn prov-btn'; btn.dataset.prov=p.name; btn.dataset.id=p.id;
    ...
    // i18n テキストを適用
    if (w.i18n) applyI18N(w.i18n);
  } catch(e) { console.warn('world.json:', e); }
}

// ── ボタン初期化（起動時に即実行）──
// 全設定はHTMLのdata属性から取得 → 国名・色のハードコードなし
// <button data-prov="伊豆" data-id="izu" data-color="60,140,70" data-autoload="true">
...
function detectGaps(){
  ...
  function mkCell(nc,nr,t,label){
    return{col:nc,row:nr,lat:Math.round((O_LAT+nr*LAT2)*1e6)/1e6,lng:Math.round((O_LNG+nc*LNG_S)*1e6)/1e6,
      hex_id:(t===7?'sea_':'gap_')+nc+'_'+nr,
      attr:{elevation_m:0,terrain_type:t,passable:t!==7,cost:t===7?9.9:1.5,is_river:false,capturable:t!==7,special:true,special_type:t===7?'sea':'border_gap',label}};
  }
...
function startGPS(){
  if(!navigator.geolocation){alert(t('ui.gps_na','GPS not supported'));return;}
  gpsActive=true;
...
async function onGPSUpdate(lat,lng,accuracy){
  ...
  let found=null;
  Object.keys(data).filter(k=>!k.startsWith('__')).forEach(n=>data[n].forEach(c=>{if(c.col===cr.col&&c.row===cr.row)found=n;}));
...
function fit(){
  ...
  const W=cv.width/DPR,H=cv.height/DPR,pad=30,sc=Math.min((W-pad*2)/(maxX-minX),(H-pad*2)/(maxY-minY),4);
...
function draw(t){
  ...
  cells.forEach(({c,n})=>{
    ...
    const h=cache.find(x=>x.c===c);if(!h)return;
    const{cx,cy,pts}=h;
    neighbors(c.col,c.row).forEach(([nc,nr])=>{
      const nb=cache.find(x=>x.c.col===nc&&x.c.row===nr);if(!nb||nb.n===n)return;
      const TOL=R*.08;
      for(let si=0;si<6;si++){const s0=pts[si],s1=pts[(si+1)%6];for(let ni=0;ni<6;ni++){const n0=nb.pts[ni],n1=nb.pts[(ni+1)%6];if((D(s0,n0)<TOL&&D(s1,n1)<TOL)||(D(s0,n1)<TOL&&D(s1,n0)<TOL)){ctx.beginPath();ctx.moveTo(s0.x,s0.y);ctx.lineTo(s1.x,s1.y);ctx.strokeStyle='rgba(0,0,0,.5)';ctx.lineWidth=1.5/vp.sc;ctx.stroke();}}}
    });
  });
  ...
  autoCells.forEach(({c,n})=>{
    ...
    cache.push({c,n:t('cell.unknown_sea'),cx,cy,pts,isWater:true,wtype:'sea'});
  });
  ...
  castleCells.forEach(({c,n})=>{
    ...
    if(R*vp.sc>12){ctx.font=`${Math.max(7,R*.6)}px serif`;ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText('🏯',cx,cy);}
  ...
  if(overlayData){
    ...
    visPortIds.forEach(pid=>{
      ...
      cache.push({c:{col:port.col,row:port.row,lat:port.lat,lng:port.lng,hex_id:'port_'+pid,attr:{terrain_type:4,elevation_m:0,passable:true,cost:1,special:true,label:port.name}},n:port.province,cx,cy,pts,isPort:true,portData:port});
    });
  }
  ...
  if(sh){
    ...
    if(sh.c&&sh.c.hex_id){
      ...
      let head=isGap?t('cell.border'):isPort?t('cell.port'):isCastle?`${t('cell.castle')} ${sh.c.attr.label}`:isWater?(wtype==='sea'?t('cell.sea'):wtype==='river'?(t('cell.river')+(sh.c.attr.flood_risk?' '+t('cell.flood'):'')):wtype==='lake'?t('cell.lake'):t('cell.sea_route')):(sh.n+(sh.isSpecial?' '+t('cell.special'):isIsland?' '+t('cell.island'):''));
...
wrap.addEventListener('touchmove',e=>{
  ...
  if(e.touches.length===1&&ts){
    const dx=e.touches[0].clientX-ts.clientX,dy=e.touches[0].clientY-ts.clientY;
    if(Math.hypot(dx,dy)>5)dd=true;
    vp.ox+=dx;vp.oy+=dy;ts=e.touches[0];
